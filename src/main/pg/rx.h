...
typedef struct rxConfig_s {
    ...
    // 1EURO fc_min/fc_max/deriv_hz are fully internal/computed-only (no CLI fields) — see
    // rcSmoothingSetFilterCutoffs() in fc_rc.c. beta is likewise fully computed with no field.
    uint8_t rc_smoothing_1euro_stage2;      // 1EURO stage 2 (static fixed-cutoff PT1) ON/OFF; OFF = single-stage adaptive-only
