...
typedef struct rxConfig_s {
    ...
    uint8_t rc_smoothing_input_type;        // Input filter type (0 = PT1, 1 = 1EURO, 2 = PT2, 3 = 2EURO)
    // 1EURO (single-stage) and 2EURO (dual-stage) are independent peer selections, same as
    // PT1/PT2 — not a toggle. fc_min/fc_max/fc_d/beta are fully internal/computed-only for both
    // (no CLI fields) — see rcSmoothingSetFilterCutoffs() in fc_rc.c.
